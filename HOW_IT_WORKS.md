...
Your **Market Prediction Analyzer** is a web application that tracks and analyzes prediction markets from Manifold Markets. It shows probability changes over time using **sparkline charts** and highlights recent movers with the catch-up digest.

---

## 🔄 Data Flow: Where Does the Data Come From?
...
- The lookback window is user-selectable; backfill runs if there is not enough local history
- For charts, the app can backfill daily points over the selected window for the most active markets
...
1. **Data Collection**: 
   ...
   - Only snapshots from the last 10 days are used for sparklines
...
Render market cards with sparklines
  ↓
Render catch-up digest and stats
```
...
```
If selected window > available history
  ↓
...
- Shows outcome type, status, close date, volume, and liquidity
...
## ⚙️ Configuration
### **Lookback Window**
- User-selectable (1-365 days)
- Stored in: `localStorage.lookback_days`
- Controls how far back charts and the catch-up digest look
...
## 📊 Example: How a Chart is Built

Let's say you have a market "Will AI surpass humans by 2030?" and you use a 10-day lookback window:

1. **Day 1**: You fetch markets → Probability is 45% → Saved to localStorage
2. **Day 2**: You fetch again → Probability is 48% → Saved
...
- Card shows: "Then: 45% → Now: 65%" with green sparkline
...
1. ✅ Live data from Manifold Markets API
2. ✅ Historical snapshots stored in your browser's localStorage
3. ✅ Automatic backfilling from Manifold's betting history API
...
- Highlights movers and trend deltas
